...
// let largeShirts = [];
let largeShirts = runners.filter(list => list.shirt_size === "L")
console.log(largeShirts);

// ==== Challenge 4: Use .reduce() ====
// The donations need to be tallied up and reported for tax purposes. Add up all the donations into a ticketPriceTotal array and log the result
// let ticketPriceTotal = [];
let ticketPriceTotal = runners.reduce((total, list) => total + list.donation, 0)
...
// Problem 1
// Need a list of Names (only) that have donated at 100 Dollars
let over100 = runners.filter(list => list.donation > 100)
	over100.forEach((list, index) => console.log(`${over100[index].first_name} ${over100[index].last_name}`))
	
// Problem 2
//A list of people with gmail email
let yahooEmail = runners.filter(list => list.email.includes("yahoo"))
console.log(yahooEmail);

// Problem 3
// an array containing each unique company name, and how many times each company is reused
let companies = runners.map(list => list.company_name)
...
companies.forEach((list, i, arrComp) => {
	if (list === arrComp[i+1]) {
        count++;
    } else {
        uniqueCompanies.push({unique_company: companies, count: count});
        count = 1;
    }
});
